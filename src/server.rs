...
impl CodeAgent {
    #[tool(
        description = "IMPORTANT: THIS MUST BE THE FIRST TOOL CALLED IN EVERY CONVERSATION.\n\n- Always call this at the start of the conversation before using any other tools from winx.\n- Use `workspace_path` to initialize the shell in the appropriate project directory.\n- If the user has mentioned a workspace or project root or any other file or folder use it to set `workspace_path`.\n- If user has mentioned any files use `initial_files_to_read` to read, use absolute paths only (~ allowed)\n- By default use mode \"wcgw\"\n- In \"code-writer\" mode, set the commands and globs which user asked to set, otherwise use 'all'.\n- Use initialization_type=\"first_call\" if it's the first call to this tool.\n- Use initialization_type=\"user_asked_mode_change\" if in a conversation user has asked to change mode.\n- Use initialization_type=\"reset_shell\" if in a conversation shell is not working after multiple tries.\n- Use initialization_type=\"user_asked_change_workspace\" if in a conversation user asked to change workspace\n"
    )]
    async fn initialize(
        &self,
