...
const IMPORTANT_PATTERNS: &[&str] = &[
    ...
    "Pipfile", "Pipfile.lock", "requirements.txt", "pyproject.toml", "setup.py",
    "Gemfile", "Gemfile.lock", "build.gradle", "pom.xml", "build.sbt",
    ".gitignore", ".editorconfig", "tsconfig.json", 
    
    // Documentation
    "README.md", "README.txt", "CONTRIBUTING.md", "CHANGELOG.md", "LICENSE", 
    
    // Source code entrypoints
    "main.rs", "lib.rs", "mod.rs", "index.js", "app.js", "app.py", "main.py",
    
    // Test directories
    "tests/", "test/", "__tests__/", "spec/",

    // Config directories 
    ".github/", ".circleci/", ".vscode/"
];

/// File patterns that should be ignored
const IGNORE_PATTERNS: &[&str] = &[
    // Build artifacts and dependencies
    "target/", "node_modules/", "dist/", "build/", "venv/", ".env/", 
    "__pycache__/", "*.pyc", "*.pyo", "*.pyd", "*.so", "*.dll", "*.dylib",
    
    // IDE files
    ".idea/", ".vscode/", ".vs/", "*.iml", "*.code-workspace",
    
    // Lock files
    "yarn.lock", "package-lock.json", "Cargo.lock",
    
    // Large data files
    "*.min.js", "*.min.css", "*.map", "*.gz", "*.zip", "*.tar", "*.jar", "*.war",
...
impl PathImportanceAnalyzer {
    ...
    pub fn initialize(&mut self) -> Result<()> {
        ...
        if let Ok(git_files) = self.get_recent_git_files(10) {
            for (path, timestamp) in git_files {
                ...
                self.importance_scores.insert(path, ImportanceScore::RecentlyModified(0.0));
            }
        }
        
        // Factor in file access statistics
        for (path, file_info) in &self.file_read_stats {
            if let Some(ImportanceScore::Regular(score)) = self.importance_scores.get(path) {
                // More reads/writes/edits = higher score
                let access_score = file_info.line_ranges.len() as f64 * 0.5 + 
                                   file_info.percentage_read() * 0.3;
        ...
        debug!("Path importance analyzer initialized with {} files", self.importance_scores.len());
        Ok(())
    }
    ...
    fn scan_workspace(&mut self) -> Result<()> {
        ...
        fn visit_dirs(dir: &Path, base: &Path, scores: &mut HashMap<PathBuf, ImportanceScore>) -> Result<()> {
            if dir.is_dir() {
    ...
    pub fn get_important_paths(&self, limit: usize) -> Vec<PathBuf> {
        ...
        paths.sort_by(|a, b| {
            b.1.value().partial_cmp(&a.1.value()).unwrap_or(std::cmp::Ordering::Equal)
        });
