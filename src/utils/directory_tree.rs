...
impl DirectoryTree {
    ...
    pub fn new(root: impl AsRef<Path>, max_files: usize) -> Result<Self> {
        ...
        if !root.is_dir() {
            return Err(anyhow::anyhow!("Root path is not a directory: {}", root.display()));
        }
    ...
    pub fn expand(&mut self, rel_path: impl AsRef<Path>) -> Result<()> {
        ...
        if !abs_path.exists() {
            return Err(anyhow::anyhow!("Path does not exist: {}", abs_path.display()));
        }
    ...
    fn count_hidden_items(&self, dir_path: &Path, shown_items: &[PathBuf]) -> Result<(usize, usize)> {
        ...
        fn display_recursive(
            ...
            if current_path.is_dir() {
                ...
                for item in &contents {
                    ...
                    let should_show = tree.expanded_files.contains(item) || 
                                     tree.expanded_dirs.contains(item);
                ...
                let (hidden_files, hidden_dirs) = tree.count_hidden_items(current_path, &shown_items)?;
                
                if hidden_files > 0 || hidden_dirs > 0 {
                    let mut hidden_message = String::new();
                    
                    if hidden_dirs > 0 {
                        write!(hidden_message, "{} director{}", 
                               hidden_dirs, 
                    ...
                    if hidden_files > 0 {
                        write!(hidden_message, "{} file{}", 
                               hidden_files,
                               if hidden_files == 1 { "" } else { "s" })?;
                    }
                    
                    writeln!(output, "{}{} hidden: {}", 
                           " ".repeat(indent + 2),
                           if depth > 0 { "..." } else { "" }, 
                           hidden_message)?;
                }
