...
impl fmt::Display for DiffOp {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            DiffOp::Replace { start, end, content } => {
                write!(f, "Replace from position {} to {} with '{}'", start, end, content)
            }
        }
    }
}
    ...
    while i < operations.len() {
        if i + 1 < operations.len() {
            match (&operations[i], &operations[i + 1]) {
                (DiffOp::Delete { start, end }, DiffOp::Insert { position, content }) if *end == *position => {
        ...
        match op {
            DiffOp::Insert { content, position } => {
                if position > result.len() {
                    return Err(anyhow::anyhow!("Insert position {} out of bounds (len: {})", position, result.len()));
                }
            ...
            DiffOp::Delete { start, end } => {
                if end > result.len() || start > end {
                    return Err(anyhow::anyhow!("Delete range {}:{} is invalid (len: {})", start, end, result.len()));
                }
            ...
            DiffOp::Replace { start, end, content } => {
                if end > result.len() || start > end {
                    return Err(anyhow::anyhow!("Replace range {}:{} is invalid (len: {})", start, end, result.len()));
                }
                result.replace_range(start..end, &content);
                debug!("Replaced from position {} to {} with '{}'", start, end, content);
            }
        }
    }
...
mod tests {
    ...
    fn test_diff_replace() {
        ...
        if let DiffOp::Replace { start, end, content } = &ops[0] {
            assert_eq!(*start, 6);
            assert_eq!(*end, 11);
