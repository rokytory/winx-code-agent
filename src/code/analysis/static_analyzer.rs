...
impl StaticAnalyzer {
                            ...
                            if let (Some(level_str), Some(message_str)) = (level.as_str(), message_text.as_str()) {
                                ...
                                if let Some(spans_array) = spans.as_array() {
                                    for span in spans_array {
                                        if let (Some(is_primary), Some(file_name), Some(line_number)) = (
                                            span.get("is_primary"),
                                            span.get("file_name"),
                                            span.get("line_start"),
                                        ) {
                                            if is_primary.as_bool() == Some(true) {
                                                ...
                                                let issue = StaticAnalysisIssue {
                                                    path: PathBuf::from(file_name.as_str().unwrap_or("")),
                                                    line: line_number.as_u64().unwrap_or(0) as usize,
                                                    column,
                                                    message: message_str.to_string(),
                                                    severity,
                                                    code: json.get("code").and_then(|c| c.get("code")).and_then(|c| c.as_str()).map(|s| s.to_string()),
    ...
    pub async fn analyze_python(&self, file_paths: &[impl AsRef<Path>]) -> Result<StaticAnalysisReport> {
        ...
        let file_path_strs: Vec<String> = file_paths.iter()
        ...
        match serde_json::from_str::<Vec<serde_json::Value>>(&stdout) {
            Ok(issues) => {
                for issue in issues {
                    if let (Some(path), Some(line), Some(message), Some(message_id), Some(symbol), Some(type_)) = (
                        issue.get("path"),
                        issue.get("line"),
                        issue.get("message"),
                        ...
                        if let (Some(path_str), Some(line_num), Some(message_str), Some(type_str)) = (
                            ...
                            let column = issue.get("column").and_then(|c| c.as_u64()).map(|c| c as usize);
    ...
    pub async fn analyze_js_ts(&self, file_paths: &[impl AsRef<Path>]) -> Result<StaticAnalysisReport> {
        let mut report = StaticAnalysisReport::new();
        report.stats.files_analyzed = file_paths.len();
    ...
    pub async fn analyze_project(&self, project_analysis: &crate::code::analysis::ProjectAnalysis) -> Result<StaticAnalysisReport> {
        let mut report = StaticAnalysisReport::new();
