...
use crate::lsp::types::{Language, SymbolLocation};
...
impl CodeManager {
    ...
    pub async fn new(lsp_server: Arc<Mutex<LSPServer>>, root_path: impl AsRef<Path>) -> Result<Self> {
    ...
    pub async fn were_lines_read(&self, relative_path: &str, start_line: usize, end_line: usize) -> bool {
    ...
    pub async fn read_file(&self, relative_path: &str, start_line: usize, end_line: Option<usize>) -> Result<String> {
    ...
    pub async fn create_file(&self, relative_path: &str, content: &str) -> Result<()> {
        ...
        if let Some(parent) = file_path.parent() {
            fs::create_dir_all(parent).await
                .context(format!("Failed to create parent directory for: {}", relative_path))?;
        }
    ...
    pub async fn replace_lines(&self, relative_path: &str, start_line: usize, end_line: usize, new_content: &str) -> Result<()> {
        // First check if the lines were read to verify the operation
        if !self.were_lines_read(relative_path, start_line, end_line).await {
