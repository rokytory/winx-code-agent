        ...
        for line in stdout.lines() {
            if let Ok(value) = serde_json::from_str::<serde_json::Value>(line) {
                if let Some(message) = value.get("message") {
                    if let (Some(level), Some(message_text)) =
    ...
    pub async fn analyze_python(
        ...
        if let Ok(pylint_issues) = serde_json::from_str::<Vec<serde_json::Value>>(&stdout) {
            for issue_value in pylint_issues {
                if let (Some(path), Some(line), Some(message), Some(message_id), Some(symbol)) = (
        ...
        match (issue.source.as_str(), issue.code.as_deref()) {
            ...
            ("eslint", Some(code)) => {
                ...
                match code {
                    ...
                    "semi" => Some("Add semicolon at the end of the line".to_string()),
                    _ => None,
                }
            }
            _ => None,
