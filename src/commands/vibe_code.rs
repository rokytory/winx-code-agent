...
pub async fn init_vibe_code(state: &SharedState, json_str: &str) -> Result<String> {
    ...
    // Parse the request
    let request: InitVibeCodeRequest = serde_json::from_str(json_str)
        .with_context(|| "Failed to parse init_vibe_code request")?;
    ...
    info!("VibeCode agent initialized with project: {}", request.project_dir);
    
    // Generate a project overview
    let agent = get_vibe_agent(state)?;
    let overview = agent.get_project_overview()
...
pub async fn analyze_file(state: &SharedState, json_str: &str) -> Result<String> {
    debug!("Analyzing file using VibeCode agent from JSON: {}", json_str);
    
    // Parse the request
    let request: AnalyzeFileRequest = serde_json::from_str(json_str)
        .with_context(|| "Failed to parse analyze_file request")?;
...
pub async fn apply_search_replace(state: &SharedState, json_str: &str) -> Result<String> {
    debug!("Applying search/replace using VibeCode agent from JSON: {}", json_str);
    
    // Parse the request
    let request: SearchReplaceRequest = serde_json::from_str(json_str)
        .with_context(|| "Failed to parse search_replace request")?;
    ...
    let result = agent.apply_search_replace(&request.file_path, &request.search_replace_blocks).await?;
...
pub async fn generate_code_suggestions(state: &SharedState, json_str: &str) -> Result<String> {
    debug!("Generating code suggestions using VibeCode agent from JSON: {}", json_str);
...
fn get_vibe_agent(state: &SharedState) -> Result<VibeAgent> {
    ...
    let agent_ref = state_guard.get_context_data::<VibeAgent>("vibe_agent")
...
fn get_vibe_agent_mut(state: &SharedState) -> Result<VibeAgent> {
    ...
    let agent = state_guard.get_context_data::<VibeAgent>("vibe_agent")
        .ok_or_else(|| anyhow!("VibeCode agent not initialized. Call init_vibe_code first."))?;
