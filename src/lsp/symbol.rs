...
impl Symbol {
    ...
    pub fn from_symbol_information(
    ...
    pub fn to_dict(
        ...
        if include_children && depth > 0 {
            let children = self
                .children
                .iter()
                .map(|child| child.to_dict(include_body, include_children, depth - 1))
                .collect::<Vec<_>>();
...
impl SymbolManager {
    ...
    pub async fn get_document_symbols(
        ...
        let symbols = match response {
            lsp_types::DocumentSymbolResponse::Nested(document_symbols) => document_symbols
                .iter()
                .map(|symbol| {
                ...
                .collect(),
            lsp_types::DocumentSymbolResponse::Flat(symbol_infos) => symbol_infos
                .iter()
                .filter_map(|symbol| Symbol::from_symbol_information(symbol, &self.root_path).ok())
                .collect(),
        };
    ...
    pub async fn find_symbol_at_location(
        &self,
        relative_path: impl AsRef<Path>,
        line: u32,
        column: u32,
    ) -> WinxResult<Option<Symbol>> {
