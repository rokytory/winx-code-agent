...
use crate::lsp::{
    ...
    utils::{format_symbol_info, read_file_content, replace_range, to_lsp_range, write_file_content},
};
...
async fn get_lsp_client(root_path: impl AsRef<Path>) -> WinxResult<Arc<TokioMutex<LspClient>>> {
    ...
    let file_types = ["rs", "py", "js", "ts", "java", "go", "rb", "cs", "cpp", "c", "h"];
    ...
    let client = Arc::new(TokioMutex::new(
        factory.create_client_for_file(file_path.unwrap(), root_path).await?,
    ));
                    ...
                    for path_result in paths.take(100) {
                        if let Ok(file_path) = path_result {
                            if let Ok(relative_path) = file_path.strip_prefix(&workspace_root) {
                                ...
                                match symbol_manager.get_document_symbols(&relative_path_str).await {
